...
class Sampling(object):
    ...
    def sample(self, batch, split, sourcesplit):
        ...
        sourcedataset = np.asarray(sourcedataset).reshape(
            -1,
        )
        ...
        self.distribution = pd.concat([self.distribution, pd.DataFrame(distribution, columns=["targetdataset", "sourcedataset"])], axis=0, ignore_index=True)
