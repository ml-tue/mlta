...
class Function(tf.keras.layers.Layer):
    def __init__(self, units, nhidden, nonlinearity, architecture, trainable):
        ...
        self.block = [tf.keras.layers.Dense(units=get_units(_, self.units, architecture, self.n), trainable=trainable) for _ in range(self.n)]
...
class FunctionF(Function):
    def __init__(self, units, nhidden, nonlinearity, architecture, trainable, resblocks=0):
        ...
        if resblocks > 0:
            ...
            self.block += [
                ResidualBlock(units=self.units, architecture=architecture, nhidden=self.n, trainable=trainable, nonlinearity=self.nonlinearity) for _ in range(resblocks)
            ]
            self.block += [tf.keras.layers.Dense(units=self.units, trainable=trainable)]
