...
from utilities.dataset2vec.modules import FunctionF, FunctionG, FunctionH, PoolF, PoolG, PoolH
...
class Model(object):
    ...
    def _create_dir(self, rootdir):
        ...
        directory = os.path.join(
            rootdir,
            "checkpoints",
            f"searchspace-{self.searchspace}",
            f"split-{self.split}",
            "dataset2vec",
            "vanilla",
            f"configuration-{self.config_num}",
    ...
    def dataset2vecmodel(self, trainable):
        ...
        layer = FunctionF(
            units=self.units_f, nhidden=self.nhidden_f, nonlinearity=self.nonlinearity_d2v, architecture=self.architecture_f, resblocks=self.resblocks_f, trainable=trainable
        )(x)
        ...
        metafeatures = FunctionH(
            units=self.units_h, nhidden=self.nhidden_h, nonlinearity=self.nonlinearity_d2v, architecture=self.architecture_h, trainable=trainable, resblocks=self.resblocks_h
        )(layer)
