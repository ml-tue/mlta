...
class MetaDataLookupTable:
    ...
    def to_csvs(self, path: str = "") -> None:
        ...
        df_pipelines = pd.DataFrame(data={"pipeline": list(self._pipeline_str_to_id.keys()), "pipeline_id": list(self._pipeline_str_to_id.values())})
        df_pipelines.to_csv(pipeline_table_path, header=["pipeline", "pipeline_id"])

        df_data = pd.DataFrame(data={"dataset": list(self._dataset_to_id.keys()), "dataset_id": list(self._dataset_to_id.values())})
